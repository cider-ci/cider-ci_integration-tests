  ...
  context 'the response of getting the project-configuration '  do
    ...
    context 'parsing error' do
      context 'response for malformed json' do
        ...
        def response_for_malformed_json
          setup_demo_repo
        ...
        it "fails with the appropriate status and message" do
          response= response_for_malformed_json
          expect(response.status).to be== 422
          expect(response.body).to match /Parser error/
        end
      ...
      context 'response for malformed yml' do
        ...
        def response_for_malformed_yml
          setup_demo_repo
          delete_all_config_files demo_repo
          demo_repo.chdir do
        ...
        it "fails with the appropriate status and message" do
          response= response_for_malformed_yml
          expect(response.status).to be== 422
          expect(response.body).to match /Parser error/
        end
